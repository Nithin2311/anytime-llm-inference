...
Also produces L16 confidence distribution histogram.
"""
...
def main():
    ...
    all_confs = [td["l16_conf"] for td in raw_tokens if "l16_conf" in td]
    thresholds = [0.40, 0.50, 0.55, 0.60, 0.70, 0.80, 0.90]
    frac_above = {
        t: round(100.0 * sum(c >= t for c in all_confs) / max(1, len(all_confs)), 2)
        for t in thresholds
    }
    print(f"\n  L16 confidence distribution ({len(all_confs)} tokens):")
    for t, f in frac_above.items():
        print(f"    c >= {t:.2f}: {f:.2f}% of tokens")

    fig2, ax3 = plt.subplots(figsize=(3.5, 2.6))
    ax3.hist(all_confs, bins=50, color="#0072B2", alpha=0.75, edgecolor="white",
             linewidth=0.3)
    for t, ls in zip([0.55, 0.70], ["--", ":"]):
        ax3.axvline(t, color="#D55E00", ls=ls, lw=1.2,
                    label=f"τ={t} ({frac_above[t]:.1f}% exits)")
    ax3.set_xlabel("L16 Softmax Confidence")
    ax3.set_ylabel("Token Count")
    ax3.set_title("L16 Confidence Distribution (500-query)")
    ax3.legend(fontsize=7)
    plt.tight_layout()
    conf_fig_path = RESULTS_DIR / "confidence_distribution.png"
    plt.savefig(conf_fig_path, dpi=150)
    plt.close()
    print(f"  Confidence histogram saved: {conf_fig_path}")
    ...
    output = {
        ...
        "conf_distribution": {
            ...
            "mean_conf": round(float(sum(all_confs) / max(1, len(all_confs))), 4),
        },
    }
