#!/usr/bin/env bash
# setup.sh — One-shot environment setup for sprint_e00 on RunPod. Safe to re-run.
# Called by: tmux_launch.sh (line "bash setup.sh") and manually by the user.
# No existing file serves this purpose in sprint_e00/.
# Data: creates results/ dir and optional volume symlink; no JSON data.
set -euo pipefail
cd "$(dirname "$0")"

echo "════════════════════════════════════════════════════"
echo " Sprint E00 — WCET Large-Spaced  —  Environment Setup"
echo " $(date)"
echo "════════════════════════════════════════════════════"
...
RESULTS="$(pwd)/results"
if [ -d "$VOLUME" ]; then
  TARGET="${VOLUME}/sprint_e00_results"
  mkdir -p "$TARGET"
  if [ -L "$RESULTS" ]; then
...
HF_TOKEN_ARG=""
[ -n "${HF_TOKEN:-}" ] && HF_TOKEN_ARG="--use_auth_token"
python3 - <<'PYEOF'
import os, sys
...
echo "[5/5] PYTHONPATH ..."
SPRINT="$(pwd)"
if ! grep -q "sprint_e00/src" ~/.bashrc 2>/dev/null; then
