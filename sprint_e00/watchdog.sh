#!/usr/bin/env bash
# watchdog.sh — Polls sprint_e00 tmux session every 2 min; auto-restarts on crash.
...
while true; do
  ...
  if tmux has-session -t "$SESSION" 2>/dev/null; then
    ...
    if [ "$DONE_COUNT" -ge "$TOTAL_EXPERIMENTS" ]; then
      wlog "E00 complete — watchdog exiting."
      rm -f results/watchdog.pid
      exit 0
    fi
  else
    ...
    DONE_COUNT=$(ls results/.*.done 2>/dev/null | wc -l || echo 0)
    if [ "$DONE_COUNT" -ge "$TOTAL_EXPERIMENTS" ]; then
      wlog "E00 already complete — watchdog exiting."
      rm -f results/watchdog.pid
      exit 0
    fi
