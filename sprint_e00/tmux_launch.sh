#!/usr/bin/env bash
# tmux_launch.sh — WiFi-resilient launcher. Creates tmux session 'sprint_e00'.
# Session survives SSH disconnects. Re-attach with: tmux attach-session -t sprint_e00
#
# Usage:
#   bash tmux_launch.sh              # fresh start
#   bash tmux_launch.sh --resume     # skip if .done flag exists
...
echo "  Est. : ~17 hours (5000 samples x 1s spacing x 12 cells)"

tmux new-session -d -s "$SESSION" -x 220 -y 50
tmux rename-window -t "${SESSION}:0" "e00-wcet"
...
tmux send-keys -t "$SESSION" "bash run_e00.sh${EXTRA_ARGS} 2>&1 | tee -a results/run_e00.log" Enter

echo ""
echo "Session '${SESSION}' started."
echo "  Attach : tmux attach-session -t ${SESSION}"
echo "  Detach : Ctrl-B then D"
echo "  Log    : tail -f results/run_e00.log"
