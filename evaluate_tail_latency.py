...
def get_anytime_latencies(prompt, deadline_ms, max_tokens=50):
    ...
    with torch.inference_mode():
        ...
        for _ in range(max_tokens):
            ...
            # --- Stage 1: Early exit (Layer 16) ---
            start.record()
            logits_early, _ = model(input_ids, exit_layer=16, use_cache=False)
            mid.record()
            # Synchronize here to get the true elapsed time before the deadline decision
            torch.cuda.synchronize()

            elapsed_early_ms = start.elapsed_time(mid)
            probs = torch.softmax(logits_early[0, -1, :], dim=-1)
            confidence, next_token_early = torch.max(probs, dim=-1)

            # --- Stage 2: Real-time deadline decision ---
            # Uses the measured early-exit time, not a hardcoded assumption.
...
if __name__ == "__main__":
    ...
    baseline_mean = np.mean(baseline_latencies[1:])
    ...
    anytime_mean = np.mean(anytime_latencies[1:])
