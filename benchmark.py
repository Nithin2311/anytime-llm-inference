...
def extract_label(generated_text):
    ...
    PubMedQA expects one of three labels. The model is prompted with
    "Answer:" so the first meaningful word should be the decision.
    Returns 'unknown' if no label is found in the first 5 tokens.
    """
    ...
    for label in ("yes", "no", "maybe"):
        if text.startswith(label):
            return label
    # Fallback: check first five whitespace-separated pieces
...
def run_pubmed_benchmark(n_samples=10, deadline_ms=45.0, max_new_tokens=15):
    ...
    global_metrics = {
        ...
        "global_mean_tpot_ms": round(float(np.mean(all_tpot)), 3),
...
if __name__ == "__main__":
    run_pubmed_benchmark(n_samples=10, deadline_ms=45.0, max_new_tokens=15)
