...
def plot_schedulability(baseline_lats, anytime_lats, base_stats, any_stats, deadline_ms):
    ...
    ax = axes[1]
    metric_names = ["P50", "P95", "P99", "Max", "Miss %"]
    base_vals = [base_stats["p50_ms"], base_stats["p95_ms"], base_stats["p99_ms"], base_stats["max_ms"], base_stats["miss_pct"]]
    any_vals  = [any_stats["p50_ms"],  any_stats["p95_ms"],  any_stats["p99_ms"],  any_stats["max_ms"],  any_stats["miss_pct"]]
    x = np.arange(len(metric_names))
    w = 0.35
    bars1 = ax.bar(x - w/2, base_vals, w, label="Baseline", color="#d9534f", edgecolor="black", linewidth=0.5)
    bars2 = ax.bar(x + w/2, any_vals,  w, label="Anytime",  color="#2b5b84", edgecolor="black", linewidth=0.5)
    ax.axhline(y=deadline_ms, color="#2e8b57", linestyle="--", linewidth=1.2, label=f"Deadline ({deadline_ms:.0f} ms)")
    ax.set_xticks(x)
    ax.set_xticklabels(metric_names)
    ax.set_ylabel("Latency (ms) / Miss Rate (%)")
    ax.set_title("Baseline vs. Anytime: Key Metrics")
