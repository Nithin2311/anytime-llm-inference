...
def plot_admission(table, sim_events):
    ...
    # ── Panel 1: Boundary curve — max admissible seq_len vs deadline ──────────
    ax1 = axes[0]
    ax1.plot(deadlines_fine, boundary, linewidth=2.5, color="#2ca02c",
             label="Max admissible seq len")
    ax1.fill_between(deadlines_fine, 0, boundary, alpha=0.15, color="#2ca02c")
    ax1.axvline(DEADLINE_MS, color="black", linestyle="--", linewidth=1.2,
                label=f"D = {DEADLINE_MS:.0f} ms")

    for sl, w in table.items():
        ax1.scatter([w], [sl], marker="o", s=55, color="#d62728", zorder=5)
        ax1.annotate(f"seq={sl}", xy=(w, sl), xytext=(w + 0.8, sl + 20),
                     fontsize=7.5, color="#d62728")

    ax1.set_xlabel("Deadline (ms)")
    ax1.set_ylabel("Max Admissible Sequence Length (tokens)")
    ax1.set_title("Admissibility Boundary\n(WCET_safe × 1.10 ≤ deadline)")
    ax1.legend(loc="upper left", fontsize=8)
    ax1.set_ylim(0, 1100)

    # ── Panel 2: Simulation scatter — admit vs reject ─────────────────────────
    ax2 = axes[1]
    ...
    if admit_pts:
        ax2.scatter(*zip(*admit_pts),  s=18, alpha=0.65, color="#2ca02c",
                    label=f"ADMIT ({n_admit})", zorder=3)
    if reject_pts:
        ax2.scatter(*zip(*reject_pts), s=18, alpha=0.65, color="#d62728",
                    label=f"REJECT ({n_reject})", zorder=3)

    ax2.plot(deadlines_fine, boundary, linewidth=1.8, color="black",
             linestyle="--", label="Admissibility boundary", zorder=4)

    ax2.set_xlabel("Deadline (ms)")
    ax2.set_ylabel("Sequence Length (tokens)")
    ax2.set_title(f"Admission Control Simulation\n(n={SIM_N_REQUESTS} random requests)")
    ax2.legend(loc="upper left", fontsize=8)
    ax2.set_ylim(0, 850)

    fig.suptitle(
        "Admission Control Analysis — TinyLlama-1.1B (RTX 6000 Ada, WCET_safe = max × 1.10)",
        fontsize=7.5, y=1.01,
    )
    plt.tight_layout()
