...
def plot_tradeoff(sweep_results):
    ...
    ln1 = ax1.plot(deadlines, miss_pct,  color=colour_miss, linewidth=2,
                   marker="o", label="Deadline Miss Rate (%)")
    ...
    ax1.set_ylim(-2, max(miss_pct) + 10 if max(miss_pct) > 0 else 15)
    ...
    ln3 = ax2.plot(deadlines, p99_tpot,  color=colour_tpot, linewidth=1.5,
                   marker="^", linestyle=":",  label="P99 TPOT (ms)", alpha=0.7)
    ax2.set_ylabel("TPOT (ms)", color=colour_tpot)
    ax2.tick_params(axis="y", labelcolor=colour_tpot)

    lns   = ln1 + ln2 + ln3
    labels = [l.get_label() for l in lns]
    ax1.legend(lns, labels, loc="upper left", fontsize=8)
    ax1.set_title("Schedulability & Latency vs. Deadline")
