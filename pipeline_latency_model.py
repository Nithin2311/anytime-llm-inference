...
def plot_pipeline(gpu_s, pcie_s, cpu_s, sync_s, pipeline, deadline_ms=DEADLINE_MS):
    ...
    # ── Panel 1: Component distribution (box plots) ────────────────────────────
    ax1 = axes[0]
    data   = [gpu_s["raw"], pcie_s["raw"], cpu_s["raw"], sync_s["raw"]]
    labels = ["T_gpu\n(22-layer\nforward)",
              "T_pcie\n(H2D DMA\n1 token)",
              "T_cpu\n(softmax+\ndecode)",
              "T_sync\n(cuda\nsync)"]
    ...
    ax1.set_title("Pipeline Component Distributions")
    ax1.set_yscale("log")

    for i, (d, lbl) in enumerate(zip(data, labels)):
        med = float(np.median(d))
        ax1.text(i + 1, med * 1.15, f"{med:.2f}", ha="center", va="bottom",
                 fontsize=7.5)
