...
def plot_calibration(records):
    ...
    # ── Panel 3: Overall agreement rate and sample stats ─────────────────────
    ax = axes[2]
    ax.axis("off")
    ...
    # High-confidence agreement (conf >= 0.5)
    l5_hc  = [r for r in records if r["conf_l5"]  >= 0.5]
    l16_hc = [r for r in records if r["conf_l16"] >= 0.5]
    l5_hc_agree  = sum(1 for r in l5_hc  if r["agree_l5"])  / len(l5_hc)  * 100 if l5_hc  else 0
    l16_hc_agree = sum(1 for r in l16_hc if r["agree_l16"]) / len(l16_hc) * 100 if l16_hc else 0

    rows = [
        ["Metric",                     "Layer 5",                            "Layer 16"],
        ["Total tokens",               str(total),                           str(total)],
        ["Overall agree (%)",          f"{100*l5_agree/total:.1f}",          f"{100*l16_agree/total:.1f}"],
        ["Mean confidence",            f"{np.mean([r['conf_l5']  for r in records]):.3f}",
                                       f"{np.mean([r['conf_l16'] for r in records]):.3f}"],
        ["Agree @ conf≥0.5 (%)",       f"{l5_hc_agree:.1f}  (n={len(l5_hc)})",
                                       f"{l16_hc_agree:.1f}  (n={len(l16_hc)})"],
        ["Tokens conf≥0.8 (%)",
            f"{100*sum(1 for r in records if r['conf_l5'] >=0.8)/total:.1f}",
            f"{100*sum(1 for r in records if r['conf_l16']>=0.8)/total:.1f}"],
    ]
    table = ax.table(
        cellText=rows[1:],
        colLabels=rows[0],
    ...
    ax.set_title("Agreement Statistics", pad=12)
